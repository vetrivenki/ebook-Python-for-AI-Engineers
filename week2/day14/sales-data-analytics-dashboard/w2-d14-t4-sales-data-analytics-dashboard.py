    ...
    df = pd.DataFrame(
        {
            ...
            "region": rng.choice(["East", "West", "North", "South"], rows),
            "product": rng.choice(["Laptop", "Mouse", "Monitor", "Keyboard"], rows),
            "units": rng.integers(1, 10, rows),
            "unit_price": rng.choice([25.0, 75.0, 250.0, 900.0], rows),
            "unit_cost": rng.choice([12.0, 40.0, 170.0, 700.0], rows),
        }
    )
...
def dashboard(df):
    ...
    df.groupby("month")["revenue"].sum().plot(
        ax=a[0, 0], marker="o", title="Monthly Revenue"
    )
    df.groupby("product")["revenue"].sum().plot.barh(
        ax=a[0, 1], title="Revenue by Product"
    )
    ...
    fig.tight_layout()
    target = OUTPUT / "sales-dashboard.png"
...
def main():
    df = load_clean()
    target = dashboard(df)
    df.to_csv(OUTPUT / "cleaned-sales.csv", index=False)
