...
def main():
    ...
    parser.add_argument(
        "--sklearn-only",
        action="store_true",
        help="Skip XGBoost-only tasks; reduce the final project.",
    )
    options = parser.parse_args()
    if not options.sklearn_only and importlib.util.find_spec("xgboost") is None:
        raise SystemExit(
            "Install requirements.txt first, or explicitly use --sklearn-only for a PARTIAL run."
        )
    files = sorted(
        BASE.glob("day*/**/w3-*.py"),
    ...
    for path in files:
        ...
        if (
            options.sklearn_only
            and "day21" in path.parts
            and int(task_id.split("-t")[1]) >= 3
        ):
    ...
    print(
        "PASS:",
        sum(r["status"] == "PASS" for r in results),
        "SKIPPED:",
