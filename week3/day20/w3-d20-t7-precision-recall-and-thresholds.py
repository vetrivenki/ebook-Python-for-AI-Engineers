...
print("Average precision (PR summary):", round(average_precision_score(y_valid, probability), 3))
rows = []
for threshold in [0.2, 0.35, 0.5, 0.65, 0.8]:
    labels = (probability >= threshold).astype(int)
    result = (threshold, precision_score(y_valid, labels, zero_division=0),
              recall_score(y_valid, labels, zero_division=0), f1_score(y_valid, labels, zero_division=0))
    rows.append(result)
...
print("Freeze this choice before a final test. Average precision is not trapezoidal PR-AUC.")
