...
with torch.inference_mode():
    for images, labels in valid:
        for truth, prediction in zip(labels, model(images).argmax(1)):
            matrix[truth, prediction] += 1
accuracy = matrix.diag().sum().item() / matrix.sum().item()
path = OUTPUT / "confusion-matrix.csv"
OUTPUT.mkdir(exist_ok=True)
with path.open("w", newline="") as file:
    csv.writer(file).writerows(
        [
            ["actual/predicted", *classes],
            *[[classes[i], *row.tolist()] for i, row in enumerate(matrix)],
        ]
    )
