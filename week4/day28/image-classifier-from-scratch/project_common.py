...
def loaders(samples=500, batch_size=32):
    ...
    ), DataLoader(valid, batch_size=batch_size, num_workers=0)
...
def pattern_data(count, seed):
    ...
    palette = torch.tensor(
        [[0.1 + 0.8 * ((i >> bit) & 1) for bit in range(3)] for i in range(8)]
...
class ImageCNN(nn.Module):
    def __init__(self, classes=10):
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(), nn.Dropout(0.2), nn.Linear(128, classes)
        )
...
def train(epochs=2, samples=500):
    ...
    seed_all()
    on_device = device()
    train_loader, valid_loader = loaders(samples=samples)
    model = ImageCNN().to(on_device)
            ...
            torch.save(
                {
                    ...
                    "dataset_version": "color-patterns-v1",
                    "normalization": "x * 2 - 1",
                },
...
def load_model(path=OUTPUT / "best-model.pt"):
    ...
    if checkpoint.get("dataset_version") != "color-patterns-v1":
        raise ValueError(
            "Old or incompatible checkpoint: retrain with the reviewed code."
        )
    model = ImageCNN(classes=len(checkpoint["classes"]))
